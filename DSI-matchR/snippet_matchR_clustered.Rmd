...
knitr::opts_chunk$set(warning = FALSE, cache=T, fig.width = 8, fig.height = 6)
...
```{r explore_datasets, exercise = TRUE, exercise.lines=2}
...
```{r max, exercise = TRUE, exercise.lines=30, eval = T}
########################BLOCK 13 ############################
trump_tweets %>% 
  filter(source %in% c("Twitter for Android","Twitter for iPhone")) %>% #filter to the two biggest sources
...
```{r summary1, exercise = TRUE, exercise.lines=39, eval = T}
################### BLOCK 5 ########################
...
```{r props, exercise = TRUE, exercise.lines=47, eval = T }
################### BLOCK 9 ########################
# Scatterplot
trump_tweets %>%
  filter(source %in% c("Twitter for Android","Twitter for iPhone","TweetDeck","Twitter Web Client")) %>% #filter to the  biggest sources
...
```{r association, exercise = TRUE, exercise.lines=41, eval = T }
################### BLOCK 9 ########################
# Scatterplot
trump_tweets %>%
  filter(source %in% c("Twitter for Android","Twitter for iPhone","TweetDeck","Twitter Web Client")) %>% #filter to the  biggest sources
...
<summary> 
click here for another couple of analyses of this data
</summary>
...
```{r words, exercise = TRUE, exercise.lines=21, eval = T }
################### BLOCK 15 ########################
wc <- tweet_words %>%
...
```{r extra_words, eval=T, warning=F}
...
## Code blocks
This is just all the code blocks from above.
